...
// In-memory dev storage for Superset credentials (not persisted)
type SupersetConfig = {
  baseUrl: string;
  username: string;
  password: string;
    ...
    try {
      ...
      try {
        ...
        const r = await client.query(
          'SELECT COUNT(*)::bigint AS cnt FROM l4k_model.joinsales_orderline'
        );
...
router.get('/superset/status', async (_req, res) => {
  ...
  try {
    const loginResp = await fetch(
      `${config.baseUrl.replace(/\/$/, '')}/api/v1/security/login`,
      {
        method: 'POST',
    ...
    const json = (await loginResp.json()) as { access_token?: string };
    const token = json?.access_token;
    const ok = typeof token === 'string' && token.length > 0;
    return res.json({ configured: true, loginOk: ok, dashboards: config.dashboards });
  } catch (err) {
    return res.status(200).json({ configured: true, loginOk: false, error: (err as Error).message });
  }
});
...
router.post('/superset/dashboards', async (req, res) => {
  ...
  const store = await loadStore();
  if (!store.superset) return res.status(400).json({ error: 'SUPERSET_NOT_CONFIGURED' });
...
router.delete('/superset/dashboards/:id', async (req, res) => {
  ...
  if (store.superset.dashboards.length === before) return res.status(404).json({ error: 'NOT_FOUND' });
  await saveStore(store);
  res.json({ ok: true });
});
